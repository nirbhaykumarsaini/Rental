...
export async function PUT(
  ...
  try {
    ...
    const minOrderQuantity =
      parseInt(formData.get("minOrderQuantity") as string) || 1;
    ...
    if (mainImages.length > 0) {
      ...
      const keepImagesArray = keepImages
        ? keepImages.split(",").filter((img) => img)
        : [];
      ...
      try {
        ...
        for (let i = 0; i < variants.length; i++) {
          ...
          if (variantImages.length > 0) {
            ...
            for (const image of variantImages) {
              if (image.size > 0) {
                try {
                  ...
                  console.error(
                    "Error uploading variant image to Cloudinary:",
                    error,
                  );
                }
              }
            ...
            const keepVariantImages = formData.get(
              `keepVariantImages_${i}`,
            ) as string;
            const keepVariantImagesArray = keepVariantImages
              ? keepVariantImages.split(",").filter((img) => img)
              : [];
            variants[i].images = [
              ...keepVariantImagesArray,
              ...uploadedVariantImages,
            ];
          }
        }

        // Clean up empty _id fields in variants and sizes
        const cleanedVariants = variants.map((variant: any) => {
          if (variant.sizes && Array.isArray(variant.sizes)) {
            variant.sizes = variant.sizes.map((size: any) => {
              // Remove _id if it's empty or invalid
              if (
                size._id === "" ||
                (size._id && !mongoose.Types.ObjectId.isValid(size._id))
              ) {
          ...
          // Also clean variant _id if empty
          if (
            variant._id === "" ||
    ...
    if (product.hasVariants && product.variants.length > 0) {
      ...
      const totalInventory = (product.variants as IProductVariant[]).reduce(
        (total: number, variant: IProductVariant) => {
          ...
          if (Array.isArray(sizes)) {
            return (
              total +
              sizes.reduce((sum: number, size: IProductVariantSize) => {
                return (
                  sum +
                  (typeof size.inventory === "number" ? size.inventory : 0)
                );
              }, 0)
            );
          }
  ...
  try {
    ...
    if (publicIds.length > 0) {
      try {
        ...
        console.log(
          `Deleted ${publicIds.length} images from Cloudinary for product ${id}`,
        );
